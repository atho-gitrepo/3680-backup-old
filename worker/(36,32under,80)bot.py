...
MINUTES_32_MINUTE_BET = [31, 32, 33] # ADDED: New constant for 32' bet timing
MINUTES_80_MINUTE_BET = [79, 80, 81]
BET_TYPE_REGULAR = 'regular'
BET_TYPE_32_UNDER = '32_under' # ADDED: New constant for the 32' under bet
...
# ADDED: New function to place the 32' under bet
def place_32_under_bet(state, fixture_id, score, match_info):
    """Handles placing the 32' under bet."""
    
    # Extract home and away goals
    try:
        home_goals, away_goals = map(int, score.split('-'))
    except ValueError:
        # If score is not in 'x-y' format, exit
        return

    # Determine the bet type and current total
    bet_info = None
    if score in ['0-4', '0-5']:
        # Bet on Team 2 (away team) under
        # The line is current total score + 0.5
        under_line = home_goals + away_goals + 0.5
        bet_info = {
            'bet_type': 'under',
            'team': 'Team 2',
            'line': under_line,
    ...
    if bet_info:
        ...
        unresolved_data = {
            ...
            'country': match_info['country'],
            'league_id': match_info['league_id'],
            'bet_type': BET_TYPE_32_UNDER, # New bet type identifier
            '32_score': score,
            'under_line': bet_info['line'],
            'fixture_id': fixture_id
        }
        firebase_manager.add_unresolved_bet(fixture_id, unresolved_data)
        
        # Send Telegram notification
        send_telegram(f"⏱️ 32' - {match_info['match_name']}\n🏆 {match_info['league_name']} ({match_info['country']})\n🔢 Score: {score}\n🎯 Bet Placed: {bet_info['team']} under {bet_info['line']}")
    else:
        # Also mark as placed to avoid re-checking on every loop
        state['32_bet_placed'] = True
        firebase_manager.update_tracked_match(fixture_id, state)

# MODIFIED: Updated to handle both 36' regular and 32' under bets
def check_ht_result(state, fixture_id, score, match_info):
    ...
    if unresolved_bet_data:
        ...
        elif bet_type == BET_TYPE_32_UNDER:
            # Logic for 32' under bet
            under_line = unresolved_bet_data.get('under_line')
            try:
                home_goals, away_goals = map(int, current_score.split('-'))
                total_goals = home_goals + away_goals
                outcome = 'win' if total_goals < under_line else 'loss'
                message = f"✅ HT Result: {match_info['match_name']}\n🏆 {match_info['league_name']} ({match_info['country']})\n🔢 Score: {current_score}\n🎉 32' Bet WON (Under {under_line})" if outcome == 'win' else f"❌ HT Result: {match_info['match_name']}\n🏆 {match_info['league_name']} ({match_info['country']})\n🔢 Score: {current_score}\n🔁 32' Bet LOST (Under {under_line})"
            except ValueError:
                # Handle cases where score format is unexpected
                outcome = 'error'
                message = f"⚠️ HT Result: {match_info['match_name']}\n❌ Bet could not be resolved due to score format issue."
...
def process_live_match(match):
    ...
    state = firebase_manager.get_tracked_match(fixture_id) or {
        '36_bet_placed': False,
        '32_bet_placed': False, # ADDED: New state flag
        '80_bet_placed': False,
        '36_score': None,
        '80_score': None,
    }
    ...
    # ADDED: New condition to place the 32' bet
    elif status.upper() == '1H' and minute in MINUTES_32_MINUTE_BET and not state.get('32_bet_placed'):
        place_32_under_bet(state, fixture_id, score, match_info)
